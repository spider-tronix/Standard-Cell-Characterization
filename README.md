...
2. **Build Netlists**:
    ...
    <insert source netlist xor2_1x.cir>
    <insert random 1>
    <insert random 2>
...
# Code Structure
1. Characterization.py : Main Script which utilises other scripts to generate the necessary output files. Also contains the function for running ngspice simulation using subprocess library.
2. NodeVoltages_.py : Analyses the logic function of the standard cell and determines the node voltages (both constant and pulse waveforms) for each netlist.
3. NetlistBuilder_.py : For Building the Netlists for different combinations and saving them in the characterization folder.
4. Characterize_.py : Analyzes the .data files to find the timing characteristics for different input nodes.[rise transition, fall transition, cell rise, cell fall].
